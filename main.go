...
import (
	...
	"github.com/kubetrail/influxdb-operator/controllers"
...
func init() {
	...
	utilruntime.Must(influxdbv1beta1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "a5b1cc23.kubetrail.io",
	})
	if err != nil {
	...
	if err = (&influxdbv1beta1.Config{}).SetupWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "Config")
		os.Exit(1)
	}
	if err = (&influxdbv1beta1.Organization{}).SetupWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "Organization")
		os.Exit(1)
	}
	if err = (&influxdbv1beta1.Bucket{}).SetupWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "Bucket")
		os.Exit(1)
	}
	if err = (&influxdbv1beta1.Token{}).SetupWebhookWithManager(mgr); err != nil {
